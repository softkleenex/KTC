...
// 브라우저는 Nginx가 배포하는 index.html을 받습니다.
// 여기서 호출하는 /api 요청은 같은 Nginx 컨테이너로 들어간 뒤,
// nginx.conf의 proxy_pass 설정을 통해 FastAPI backend 컨테이너로 전달됩니다.
async function checkApiStatus() {
    try {
        ...
        apiStatus.textContent = `API 연결됨: ${data.service}`;
        apiStatus.classList.add('connected');
    } catch (error) {
        console.error('API 상태 확인 중 오류 발생:', error);
        apiStatus.textContent = 'API 연결 실패: Nginx proxy 또는 FastAPI backend를 확인하세요.';
